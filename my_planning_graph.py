...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            admissible = True
            for literal in a.precond_pos: 
                if PgNode_s(literal, True) not in self.s_levels[level]: 
                    admissible = False
            for literal in a.precond_neg: 
                if PgNode_s(literal, False) not in self.s_levels[level]: 
                    admissible = False
            if admissible: 
                ...
                for s_node in self.s_levels[level]: 
                    if s_node.literal in (set(a.precond_pos) | set(a.precond_neg)):
    ...
    def add_literal_level(self, level):
        ...
        # 2. connect the nodes
        # for example, every A node in the previous level has a_node list of S nodes in effnodes that represent the effect
        ...
        for a_node in self.a_levels[level - 1]: 
            for literal in (set(a_node.action.effect_add) | set(a_node.action.effect_rem)): 
                if literal in a_node.action.effect_add: 
                    s_node = PgNode_s(literal, True)
                else: 
                    s_node = PgNode_s(literal, False)
    ...
    def h_levelsum(self) -> int:
        ...
        remaining_goals = set(self.problem.goal)
        for i, s_level in enumerate(self.s_levels): 
            literals = {s_node.literal for s_node in s_level}
            level_goals = remaining_goals & literals
            level_sum += i * len(level_goals)
            remaining_goals -= level_goals
            # if all the goals are met, there is no reason to continue 
